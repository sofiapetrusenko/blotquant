...
- **`dynamic_range_min_peak_fraction = 0.25`**, where 0.40 would score perfectly. Shipped gives recall **0.700**; 0.40 gives recall **1.000** with precision 1.000 at both. 0.40 was refused because it sits above the generator's scratch amplitude, so clearing it would be tuning to an artifact only this generator produces. Whether that is the right trade for a QC tool — where under-warning is the more dangerous direction — was **ruled at Phase 3 Gate 1: the threshold stays at 0.25 and the question is closed on synthetic data**, because real blots cannot select a threshold either and the whole disagreement is about a generator constant.
...
- **The geometric `overlapping` flag scores 0.000 by construction, and what little it does fire on is mostly spurious.** Because unresolved doublets yield one ROI, there is usually nothing to overlap: at the shipped threshold of 0.05, only 3 detected same-lane band pairs across the whole dev split overlap enough to flag, putting the flag on 6 detected bands — against 104 truth `overlapping` labels, none of which it matches. Reconciling that 6 with the QC table's single false positive: **only 1 of the 6 is a matched detection; the other 5 are detections that match no truth band at all.** So the flag fires predominantly on detection false positives — two spurious peaks in the same lane overlapping each other — rather than on real bands. That is worth knowing: as it stands the flag is closer to a weak detector-artifact signal than a band-overlap warning. (The generator labels overlap at a different threshold, 0.15, where the pipeline finds 2 such pairs; the shipped 0.05 is chosen from an independent criterion, not copied.) **Phase 3 Gate 1 ruled that the flag is kept as a recorded negative result, not as functionality** — retiring it would delete the evidence that a plausible-sounding QC flag can fail completely. Do not read it as a working band-overlap warning; `unresolved_shoulder` is the flag that answers that question.
- **The shipped `total_protein` mode can fail outright on an individual lane.** Its denominator is the lane's own signal integral over a full-height ROI (~9000 px), so it accumulates the background estimator's residual bias over an area far larger than any band. On one dev image (`dev_03`) that integral comes out **negative**, −5411 DN, and that lane then yields *no ratios* — recorded as `lane_denominator_not_positive` rather than returning a nonsense number; the image's other three lanes are unaffected and it emits 7 ratios in total. **Phase 3 Gate 1 ruled that the denominator stays as it is.** What follows from that ruling is a retraction rather than a change: **the claim that total-protein normalization is *better* than single-housekeeping is not supported by measurement.** What is supported is narrower — it needs no oracle reference, where the housekeeping modes require the caller to name a loading control. The 31.65% mean error is a direct consequence of integrating background-estimator residual over a full-height lane.
- **`low_dynamic_range` misses 3 of the 10 genuinely low-range dev images** (recall 0.700, precision 1.000). Under-warning is the more dangerous direction for a QC flag, and the threshold that would catch all ten was refused as tuning to the generator — see the two-thresholds note above. **Phase 3 Gate 1 closed this**: the threshold stays at 0.25 and the under-warning is recorded as a known limitation rather than an open decision. What Gate 1 did *not* rule on is replacing the underlying measure — a robust upper quantile of band peaks instead of the brightest band's peak — which remains Phase 3 work.
...
- **No real-blot validation.** Nothing here has been checked against a real western blot or against ImageJ. That is Phase 3. Gate 1 (eval design) has now ruled, including fixing the ImageJ agreement thresholds before any measurement is run; Gate 2 (the CC-BY image list) has not.
- **Most parameters were selected on the dev split.** 13 of the 20 shipped numeric parameters come from dev-split sweeps. PLAN.md puts a human eval-design gate *before* parameter iteration; that gate ran late and **has now ratified the selection as a procedure rather than re-running it** (Phase 3 Gate 1), on the grounds that re-selecting on the same split with the same selector would change nothing. A further five QC parameters were chosen from stated criteria rather than from sweeps, and are **not** among those thirteen: the gate ratified all five: `qc.shoulder_half_width_ratio` — which carries the weakest *justification* among shipped parameters, because its criterion restates its value — and `qc.dynamic_range_min_peak_fraction` individually, and the remaining three on their recorded criteria. **18 of the 20 shipped parameters are ratified**; the two left out are the single-lane pair no sweep can reach, which have the weakest *evidential basis* and which the gate judged the more serious of the two weaknesses. Note the weakest *evidential basis* is a different pair: the two single-lane parameters no sweep can reach, which the gate judged the more serious of the two weaknesses. See [DEBT.md](DEBT.md) S3 and S12.
...
real-image list. The first has ruled; its rulings are recorded in NOTES.md.
